...
// 改善キュー一覧
router.get("/queues/improvements", async (_req: Request, res: Response) => {
  try {
...
// 改善キュー統計
router.get("/queues/improvements/stats", async (_req: Request, res: Response) => {
  try {
    const items = await improvementQueue.getAll();
    const stats = {
      total: items.length,
...
// Claude確認キュー
router.get("/queues/confirmations", async (_req: Request, res: Response) => {
  try {
...
// GLM変更履歴
router.get("/queues/glm-changes", (_req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      stats,
      data: changes.slice(0, 100).map((change) => ({
        id: change.id,
