...
export class RateLimitManager {
  ...
  private onRateLimitCallback?: (isActive: boolean, details: string) => Promise<void>;
   ...
   * レートリミット検出時に呼び出し
   */
  async recordRateLimitHit(source: string): Promise<void> {
    const now = new Date();
    this.state.consecutiveHits++;

    // 冷却期間を計算（指数バックオフ、最大30分）
    const cooldownMs = Math.min(
      INITIAL_COOLDOWN_MS * Math.pow(2, this.state.consecutiveHits - 1),
      MAX_COOLDOWN_MS
    );
  ...
  private async clearRateLimit(): Promise<void> {
    ...
    if (this.onRateLimitCallback) {
      await this.onRateLimitCallback(
        false,
        `冷却期間が終了しました。累計連続ヒット: ${wasConsecutiveHits}回`
      );
    }
  }
