...
export type NotificationItemId =
  ...
  // 提案関連
  | 'suggestion_response'
  | 'suggestion_auto_reject'
  | 'suggestion_re_evaluate'
  | 'suggestion_auto_implement'
  | 'suggestion_implement_failed'
  // 戦略関連
  ...
  | 'strategy_auto_activated'
  | 'strategy_performance_warning'
  | 'strategy_roi_warning'
  | 'strategy_error_escalation'
  | 'strategy_execution_abort'
  // 損失制限
  | 'loss_limit_reached'
  // 改善関連
  ...
  | 'zenn_auth_required'
  | 'zenn_article_ready'
  | 'platform_post_failed'
  | 'article_publish_ready'
  | 'job_application_ready'
  | 'product_listing_ready';
...
// すべての通知項目の定義
export const NOTIFICATION_ITEMS: NotificationItemMeta[] = [
  // システム関連
...
export interface NotificationHistoryEntry {
  ...
  sent: boolean;
  reason?: string;
  itemId?: NotificationItemId; // 個別項目ID
}
...
const DEFAULT_SETTINGS: NotificationSettings = {
  ...
  ) as Record<NotificationItemId, boolean>,
};
...
// 項目IDが有効かどうかをチェック
function isItemEnabled(settings: NotificationSettings, itemId: NotificationItemId): boolean {
  // 項目ごとの設定があればそれを使用
...
export interface DiscordMessage {
  ...
  timestamp?: Date;
  itemId?: NotificationItemId; // 個別項目ID（オプション）
}
...
export class DiscordNotifier {
  ...
  async sendInfo(title: string, description?: string, itemId?: NotificationItemId): Promise<boolean> {
    return this.send({ type: 'info', title, description, itemId });
  }

  async sendSuccess(title: string, description?: string, itemId?: NotificationItemId): Promise<boolean> {
    return this.send({ type: 'success', title, description, itemId });
  }

  async sendWarning(title: string, description?: string, itemId?: NotificationItemId): Promise<boolean> {
    return this.send({ type: 'warning', title, description, itemId });
  }

  async sendError(title: string, description?: string, itemId?: NotificationItemId): Promise<boolean> {
  ...
  async sendSuggestionResponse(title: string, description?: string, fields?: DiscordMessage['fields'], itemId?: NotificationItemId): Promise<boolean> {
